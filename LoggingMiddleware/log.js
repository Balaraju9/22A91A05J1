// Your actual access token from step 6
const accessToken = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..HXl8fiMyWhLRuhGezwHVHwolnCzDSpnsRtIoTldInBM";

async function logToServer(stack, level, message) {
  const apiUrl = "http://20.244.56.144/evaluation-service/logs";

  const requestBody = {
    stack: stack,     // must be lowercase like "frontend"
    level: level,     // must be lowercase like "info"
    message: message
  };

  try {
    const response = await fetch(apiUrl, {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
        "Authorization": `Bearer ${accessToken}`
      },
      body: JSON.stringify(requestBody)
    });

    const text = await response.text(); // get full text response
    try {
      const data = JSON.parse(text);
      console.log("✅ Log response:", data);
    } catch {
      console.log("⚠️ Non-JSON response from server:", text);
    }
  } catch (error) {
    console.error("❌ Error sending log:", error);
  }
}
